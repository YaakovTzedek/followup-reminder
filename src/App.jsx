...
import { Phone, MessageCircle, Bell, Clock, CheckCircle2, X, AlertCircle, Calendar, Inbox } from 'lucide-react';

/* =====================================================================
   FOLLOW-UP REMINDER · MONDAY.COM BOARD VIEW APP
   ---------------------------------------------------------------------
   For Monday deployment: set USE_MOCK = false and uncomment the SDK
   integration block in `useMondayData`. Requires monday-sdk-js.
   Filtering logic: shows items whose People column includes the
   currently-logged-in user (email match via Monday SDK).
   ===================================================================== */

const USE_MOCK = true;

// In production, replace with: import mondaySdk from "monday-sdk-js"; const monday = mondaySdk();
// Column IDs are placeholders — replace with your real board column IDs.
const COLUMN_IDS = {
  followUpDate: 'date_followup',
  assignee: 'people',
  phone: 'phone',
...
/* ---------- MOCK DATA (DEMO ONLY) ---------- */
const MOCK_CURRENT_USER = { id: 'me', name: 'יעקב', email: 'yaakov@bma.org.il' };

function makeMockItems() {
  const t = Date.now();
  return [
    { id: '1', name: 'דנה כהן', phone: '052-1234567',
      followUpAt: new Date(t + 7_000), assignedTo: ['me'],
      lastContact: 'אתמול · שלחנו הצעת מחיר',
      source: 'AI לעובדי הוראה',
      script: 'דנה היי 😊 רציתי לברר אם הספקת לעיין בהצעת המחיר ששלחתי אתמול?' },
    { id: '2', name: 'משה לוי', phone: '054-7654321',
      followUpAt: new Date(t + 60 * 60_000), assignedTo: ['me'],
      lastContact: 'לפני 3 ימים · שיחת היכרות',
      source: 'אוטומציה עסקית',
      script: 'משה, מה שלומך? רוצה לתאם שיחה קצרה על השלבים הבאים?' },
    { id: '3', name: 'רחל אברהמי', phone: '050-9988776',
      followUpAt: new Date(t + 3 * 60 * 60_000), assignedTo: ['me'],
      lastContact: 'לפני שבוע · ביקשה לחזור אליה',
      source: 'דיגיטל מרקטינג',
      script: 'רחל, חשבתי עליך — יצא מסלול חדש שמתאים בדיוק למה שחיפשת.' },
    { id: '4', name: 'אבי גולן', phone: '053-1112222',
      followUpAt: new Date(t + 26 * 60 * 60_000), assignedTo: ['me'],
      lastContact: 'לפני יומיים · התלבט בין שני מסלולים',
      source: 'עיצוב גרפי',
      script: 'אבי, מה החלטת? אשמח לעזור לסגור את ההתלבטות.' },
  ];
}
...
function useMondayData() {
  ...
  const refresh = useCallback(async () => {
    if (USE_MOCK) {
      ...
      setItems(makeMockItems());
      return;
    }
    /* === PRODUCTION INTEGRATION ===
    const { data: ctx } = await monday.get('context');
    const { data: meRes } = await monday.api(`query { me { id email name } }`);
    const me = meRes.me;
    setUser(me);
    ...
    const q = `query ($id: [ID!]) {
      boards(ids: $id) {
        items_page(limit: 200) {
          items {
            id name
            column_values(ids: ["${COLUMN_IDS.followUpDate}","${COLUMN_IDS.assignee}","${COLUMN_IDS.phone}","${COLUMN_IDS.lastContact}","${COLUMN_IDS.source}","${COLUMN_IDS.script}"]) {
              id type text value
            }
          }
        }
    ...
    const { data } = await monday.api(q, { variables: { id: [boardId] } });
    ...
    const mapped = raw.map(it => {
      ...
      const assigneeVal = JSON.parse(col(COLUMN_IDS.assignee).value || '{}');
      const followUpAt = dateVal ? new Date(`${dateVal.date}T${dateVal.time || '09:00:00'}`) : null;
      const assignedTo = (assigneeVal.personsAndTeams || []).map(p => String(p.id));
      return {
        id: it.id, name: it.name, followUpAt, assignedTo,
        phone: col(COLUMN_IDS.phone).text,
        lastContact: col(COLUMN_IDS.lastContact).text,
        source: col(COLUMN_IDS.source).text,
        script: col(COLUMN_IDS.script).text || '',
      };
    }).filter(it => it.followUpAt && it.assignedTo.includes(String(me.id)));
    setItems(mapped);
    */
  }, []);
  ...
  useEffect(() => {
    refresh();
    const i = setInterval(refresh, 60_000); // re-sync every minute
...
const relative = (d) => {
  ...
  if (diff < 0) {
    ...
    return `איחור ${Math.floor(ago / 3_600_000)} שע׳`;
  }
...
const waLink = (phone, msg) => {
  ...
  return `https://wa.me/972${clean}?text=${encodeURIComponent(msg)}`;
};

/* ---------- MAIN ---------- */
export default function App() {
  const { items, user } = useMondayData();
  ...
  // detect due (assigned to current user, not done, not snoozed, time reached)
  useEffect(() => {
    if (activeAlert || !user) return;
    const myId = String(user.id);
    const due = items
      .filter(it => it.assignedTo.includes(myId) && !done[it.id])
  ...
  const complete = () => {
    if (!activeAlert) return;
    setDone(d => ({ ...d, [activeAlert.id]: true }));
    /* === PRODUCTION: write-back to Monday ===
    monday.api(`mutation {
      change_simple_column_value(item_id: ${activeAlert.id}, board_id: ${ctx.boardId},
        column_id: "${COLUMN_IDS.followUpDate}", value: "") { id }
    }`);
    */
    setActiveAlert(null);
  };
    ...
    <div dir="rtl" className="min-h-screen" style={{
        ...
        <div className="space-y-2.5">
          {upcoming.map(item => {
            ...
            return (
              <div
                key={item.id}
                className="bg-white rounded-2xl p-4 flex items-center gap-3.5 transition-all"
                style={{
                  border: due ? '1px solid #fbbf24' : '1px solid #e2e8f0',
                  boxShadow: due
                    ? '0 0 0 3px rgba(251,191,36,0.15), 0 4px 16px -4px rgba(245,158,11,0.3)'
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5 truncate">{item.lastContact}</p>
                </div>
          ...
          {upcoming.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-slate-400 text-xs mt-1">תוסיף תאריך פולואפ לליד בבורד והוא יקפוץ פה</p>
            </div>
          )}
        </div>

        {USE_MOCK && (
          <div className="mt-6 p-3.5 rounded-2xl bg-indigo-50/70 border border-indigo-100 text-xs text-indigo-900">
            <p className="font-semibold mb-0.5">⚡ מצב הדגמה</p>
            <p className="opacity-80 leading-relaxed">הפופאפ הראשון יקפוץ בעוד כ-7 שניות. בייצור — תאריכי הפולואפ נשלפים מ-Monday.</p>
          </div>
        )}
      </main>
      ...
      {activeAlert && (
        ...
        >
          ...
          >
            {/* Header strip */}
            <div className="px-5 py-3.5 flex items-center gap-3 relative overflow-hidden" style={{
              ...
              <div className="flex-1">
                ...
                <div className="text-white/85 text-[11px]">
                  {hhmm(activeAlert.followUpAt)} · {activeAlert.source}
                </div>
              </div>
            ...
            <div className="p-5">
              ...
              <div className="bg-slate-50 rounded-xl p-3 mb-2.5 text-sm text-slate-700">
                <div className="font-semibold text-slate-400 text-[10px] uppercase tracking-wider mb-1">אינטראקציה אחרונה</div>
                {activeAlert.lastContact}
              </div>
              ...
              {activeAlert.script && (
                ...
                >
                  <div className="font-semibold text-emerald-700 text-[10px] uppercase tracking-wider mb-1">הצעת הודעה</div>
                  <div className="text-slate-700 leading-relaxed">"{activeAlert.script}"</div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <a
                  href={waLink(activeAlert.phone, activeAlert.script)}
                  target="_blank"
                  rel="noreferrer"
                  onClick={complete}
                  className="flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-white text-sm transition hover:scale-[1.02] active:scale-95"
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                <button
                  onClick={() => snooze(5)}
                  className="py-2.5 rounded-xl text-[13px] text-slate-600 hover:bg-slate-100 transition border border-slate-200"
                >
                  דחה 5 דק׳
                </button>
                <button
                ...
                <button
                  onClick={complete}
                  className="py-2.5 rounded-xl text-[13px] font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition border border-emerald-200"
                >
